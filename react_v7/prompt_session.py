# -*- coding: utf-8 -*-
"""react_v7：一体化运行机制 + ReAct + 文件工具 + done 验收（第 08 篇）。"""
...
_V7 = Path(__file__).resolve().parent
...
def _import_v7_module(short: str):
    """机制查看器会先加载 v6，须用独立模块名避免 runtime_trace 等被 v6 缓存覆盖。"""
    qual = f"react_v7_{short}"
    if qual in sys.modules:
        return sys.modules[qual]
    path = _V7 / f"{short}.py"
    spec = importlib.util.spec_from_file_location(qual, path)
    if spec is None or spec.loader is None:
        raise ImportError(f"无法加载 react_v7/{short}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[qual] = mod
    v7s = str(_V7)
    if sys.path[0] != v7s:
        sys.path.insert(0, v7s)
...
_runtime_trace = _import_v7_module("runtime_trace")
RuntimeTrace = _runtime_trace.RuntimeTrace
_task_router = _import_v7_module("task_router")
TaskRoute = _task_router.TaskRoute
analyze_task = _task_router.analyze_task
_tool_registry = _import_v7_module("tool_registry")
TOOL_SCHEMAS = _tool_registry.TOOL_SCHEMAS
run_tool = _tool_registry.run_tool
_tool_steps = _import_v7_module("tool_steps")
ToolStep = _tool_steps.ToolStep
_workspace = _import_v7_module("workspace")
...
def _observation_ok(observation: str, tool_name: str) -> bool:
    """供 ToolStep.ok 与机制查看器：普通工具看「错误：」前缀；done 另解析 JSON 的 ok。"""
...
class DeliveryAgentSession(ReflectionAgentSession):
    """v7：工作区读写 + 结构化交付与 done 机器验收。"""
    ...
    def _run_react(self, user_text: str) -> str:
        """ReAct 主循环：与 v6 结构相同；v7 增量为读写/done 工具与 last_done_ok。"""
        ...
        for i in range(max_steps):
            ...
            for tc in tool_calls:
                ...
                ok = _observation_ok(observation, fn.name)
                if fn.name == "done":
...
CommercialAgentSession = DeliveryAgentSession
ReactAgentSession = DeliveryAgentSession
