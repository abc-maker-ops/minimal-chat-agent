...
class RuntimeTrace:
    ...
    def build_lines(self) -> list[str]:
        ...
        lines.append(
            ...
            "+ tool_choice / parallel + 文件工具 + done"
        )

        if self.tool_policy_summary:
            lines.append(f"   工具策略：{self.tool_policy_summary}")
        ...
        parallel_note = f"，并行批 {self.parallel_batches} 组" if self.parallel_batches else ""
        lines.append(
            f"④ ReAct：{self.react_steps} 轮 LLM，工具 {self.tool_count} 次{parallel_note}"
