# -*- coding: utf-8 -*-
"""langgraph_v11：LC StructuredTool + 并行 tools + Checkpoint/人审（第 11 篇）。"""
...
_rt_spec = importlib.util.spec_from_file_location(
    "langgraph_v11_runtime_trace", _V11 / "runtime_trace.py"
)
if _rt_spec is None or _rt_spec.loader is None:
    raise ImportError("无法加载 langgraph_v11/runtime_trace.py")
_rt_mod = importlib.util.module_from_spec(_rt_spec)
sys.modules["langgraph_v11_runtime_trace"] = _rt_mod
...
def _default_approval(payload: dict[str, Any]) -> bool:
    """终端交互审批；AUTO_APPROVE=1 时直接通过。"""
    if os.getenv("AUTO_APPROVE", "1").strip().lower() in ("1", "true", "yes"):
        return True
    names = payload.get("tool_names") or []
    msg = payload.get("message") or "是否批准本批工具？"
    print(f"\n[人审] {msg}")
    print(f"[人审] 工具: {', '.join(names)}")
    ans = input("输入 y 批准 / n 拒绝：").strip().lower()
    return ans in ("y", "yes", "是", "批准")


class LangGraphV11Session(ReflectionAgentSession):
    """v11：StructuredTool 绑定 + tools 并行 + Checkpoint/人审。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.approval_fn = approval_fn or _default_approval
        ...
        self.last_parallel_group: int | None = None
    ...
    def _clear_graph_state(self) -> None:
        ...
        self.last_node_path = ()
        self.last_parallel_group = None

    def _checkpoint_enabled(self) -> bool:
        return os.getenv("LANGGRAPH_CHECKPOINT", "").strip().lower() in (
            "1",
            "true",
            "yes",
        ) or os.getenv("HUMAN_GATE", "").strip().lower() in ("1", "true", "yes")
    ...
    def _run_langgraph(self, user_text: str) -> str:
        ...
        initial: AgentState = {
            ...
            "pending_tool_names": [],
            "human_approved": None,
            "last_parallel_group": None,
        }

        use_ckpt = self._checkpoint_enabled()
        ...
        self.last_parallel_group = final_state.get("last_parallel_group")
...
# 机制查看器与系列命名兼容
LangGraphAgentSession = LangGraphV11Session
CommercialAgentSession = LangGraphV11Session
GraphAgentSession = LangGraphV11Session
