# -*- coding: utf-8 -*-
"""复用 react_v8 工具、策略与工作区（第 11 篇不重写业务工具）。"""
...
for p in (str(_LAB / "minimal_chat_v1"), str(_V8), str(_V11)):
    if p not in sys.path:
        sys.path.insert(0, p)


def _load_v8(name: str, filename: str):
    qual = f"langgraph_v11_bridge_{name}"
...
_parallel = _load_v8("parallel_runner", "parallel_runner.py")
...
can_run_parallel = _parallel.can_run_parallel
PARALLEL_SAFE_TOOLS = _parallel.PARALLEL_SAFE_TOOLS
